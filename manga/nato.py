...
class library(lib.library):
    ...
    def search(self, expr):
        values = {"searchword": expr.lower().replace(" ", ")")}
        ...
        for obj in rc:
            if "link_story" in obj:
                m = self.urlpat.match(obj["link_story"])
                if m is None:
                    raise Exception("unexpected manga url: %s" % (obj["link_story"],))
                try:
                    yield self.byid(m.group(1))
                except KeyError:
                    pass
